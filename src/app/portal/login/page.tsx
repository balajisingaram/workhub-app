...
export default function PortalLoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] flex items-center justify-center px-4">
      ...
      {/* Small back link — discreet, not a full navbar */}
      <div className="fixed top-5 left-6">
        <Link href="/" className="text-xs text-[var(--ink4)] hover:text-[var(--ink3)] transition-smooth flex items-center gap-1.5">
          ← Back to GigHub
        </Link>
      </div>
      ...
      <div className="w-full max-w-[400px]">
        ...
        {/* Logo area — white-label ready */}
        <div className="text-center mb-10">
          <div className="w-14 h-14 rounded-2xl bg-[var(--ink)] flex items-center justify-center font-display font-black text-xl text-white mx-auto mb-4">
            A
          </div>
          <h1 className="font-display font-black text-2xl text-[var(--ink)] tracking-tight mb-1">
            Agency portal
          </h1>
          <p className="text-sm text-[var(--ink3)]">Sign in to your private staff management portal</p>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-[var(--border2)] p-8 shadow-soft">
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            ...
            {error && (
              <div className="text-xs text-[#c94040] bg-[#fde8e8] px-4 py-3 rounded-xl border border-[rgba(201,64,64,.2)]">
                {error}
              </div>
            )}
        ...
        <p className="text-center text-xs text-[var(--ink4)] mt-6 leading-relaxed">
          This is a private portal for authorised company staff only.<br />
          Not an employee? <Link href="/find-jobs" className="text-[var(--accent)] hover:underline">Find jobs on GigHub</Link>
        </p>
      </div>
    </div>
  )
