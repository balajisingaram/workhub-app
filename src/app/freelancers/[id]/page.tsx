            ...
            <div>
              ...
              <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border2)] p-8 mb-6">
                <div className="flex gap-6 items-start">
                  {/* Avatar — now real photo */}
                  <div className="relative shrink-0">
                    <img src={fl.avatarUrl} alt={fl.name}
                      className="w-24 h-24 rounded-full object-cover border-4 border-white shadow-medium"
                      onError={e => { (e.target as any).src = `https://ui-avatars.com/api/?name=${fl.name}&background=ece9e3&color=7a756c&size=96` }} />
                    {fl.isOnline && (
                      <div className="absolute bottom-1 right-1 w-5 h-5 rounded-full bg-[var(--accent)] border-[3px] border-white shadow-soft" />
                    )}
                  </div>
                  {/* Info */}
                ...
                <div className="p-8">
                  ...
                  {activeTab === 'portfolio' && (
                    <div>
                      {/* Video intro */}
                      <div className="mb-6">
                        <h3 className="font-semibold text-[var(--ink)] mb-3">Video introduction</h3>
                        <div className="relative rounded-2xl overflow-hidden bg-[var(--ink)] aspect-video flex items-center justify-center cursor-pointer group">
                          <img src="https://images.unsplash.com/photo-1611532736597-de2d4265fba3?w=800&q=80" alt="Video thumbnail" className="w-full h-full object-cover opacity-60" />
                          <div className="absolute inset-0 flex items-center justify-center">
                            <div className="w-16 h-16 rounded-full bg-white/90 flex items-center justify-center text-2xl shadow-large group-hover:scale-110 transition-smooth">▶</div>
                          </div>
                          <div className="absolute bottom-4 left-4 text-white text-sm font-medium">2:34 — About me & my work</div>
                        </div>
                      </div>
                      {fl.portfolioItems.length > 0 ? (
                        <div className="grid sm:grid-cols-2 gap-4">
                          {fl.portfolioItems.map(p => (
                            <div key={p.id} className="rounded-2xl overflow-hidden border border-[var(--border2)] group cursor-pointer">
                              <div className="h-44 bg-[var(--bg2)] overflow-hidden">
                                <img src={p.imageUrl} alt={p.title} className="w-full h-full object-cover group-hover:scale-105 transition-all duration-500" />
                              </div>
                              <div className="p-4">
                        ...
                        <div className="text-center py-12 text-[var(--ink3)]">No portfolio items added yet</div>
                      )}
                    </div>
                  )}
                  ...
                  {activeTab === 'availability' && (
                    <div>
                      <h3 className="font-semibold text-[var(--ink)] mb-5">Available time slots</h3>
                      <div className="grid grid-cols-7 gap-2 mb-6">
                        {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((day, i) => {
                          const available = i < 5
                          return (
                            <div key={day} className={`rounded-xl p-3 text-center border ${available ? 'bg-[var(--accent-light)] border-[rgba(26,107,60,.2)]' : 'bg-[var(--bg)] border-[var(--border2)]'}`}>
                              <div className={`text-xs font-bold mb-1 ${available ? 'text-[var(--accent)]' : 'text-[var(--ink3)]'}`}>{day}</div>
                              <div className={`text-[10px] ${available ? 'text-[var(--accent)]' : 'text-[var(--ink4)]'}`}>{available ? '✓' : '—'}</div>
                            </div>
                          )
                        })}
                      </div>
                      <div className="bg-[var(--bg)] rounded-xl p-5 mb-5">
                        <div className="text-xs font-bold text-[var(--ink3)] uppercase tracking-wider mb-3">Working hours</div>
                        ...
                        <p className="text-xs text-[var(--ink3)] mt-2">{fl.timezone} — Mon to Fri, 9am to 6pm</p>
                      </div>
                      <div className="flex items-start gap-3 p-4 bg-[var(--accent-light)] rounded-xl border border-[rgba(26,107,60,.15)]">
                        <span className="text-base">📅</span>
                        <div className="text-xs text-[var(--accent)] leading-relaxed">
                          <strong>Response time: {fl.responseTime}</strong><br />
                          Send a message to discuss your specific schedule requirements.
                        </div>
                      </div>
                    </div>
                  )}
          ...
          <div className="bg-[var(--surface)] rounded-3xl p-8 w-full max-w-[480px] shadow-large animate-scale-in relative">
            ...
            <div className="flex flex-col gap-4">
              ...
              <Link href="/dashboard/client" onClick={() => setHireOpen(false)}>
                <button className="w-full py-3.5 rounded-full bg-[var(--accent)] text-white font-semibold text-sm hover:bg-[var(--ink)] transition-smooth">
                  Confirm booking →
                </button>
              </Link>
            </div>
